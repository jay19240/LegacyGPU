...
interface Gfx3ParticlesOptions {
  ...
  velocityStyle: Gfx3VelocityStyle;
  velocityBase: vec3;
  velocitySpread: vec3;
...
class Gfx3Particles extends Gfx3Drawable {
  ...
  constructor(options: Partial<Gfx3ParticlesOptions>) {
    ...
    this.velocityStyle = options.velocityStyle ?? Gfx3VelocityStyle.CLASSIC;
   ...
   * 
   * @param {vec3} positionBase - The position base.
   */
  setPositionBase(positionBase: vec3): void {
    this.positionBase = positionBase;
  }

  /**
   * Set the velocity base.
   * 
   * @param {vec3} velocityBase - The velocity base.
   */
  setVelocityBase(velocityBase: vec3): void {
   ...
   * Set the color base.
   * 
   * @param {vec3} colorBase - The color base.
   */
  setColorBase(colorBase: vec3): void {
   ...
   * Set the size base.
   * 
   * @param {number} sizeBase - The size base.
   */
  setSizeBase(sizeBase: number): void {
    this.sizeBase = sizeBase;
  }

  /**
   * Set the opacity base.
   * 
   * @param {number} opacityBase - The opacity base.
   */
  setOpacityBase(opacityBase: number): void {
  ...
  #createParticle(): Particle {
    ...
    else if (this.positionStyle == Gfx3PositionStyle.SPHERE) {
    ...
    if (this.velocityStyle == Gfx3VelocityStyle.CLASSIC) {
      particle.velocity = UT.VEC3_SPREAD(this.velocityBase, this.velocitySpread);
    }
    else if (this.velocityStyle == Gfx3VelocityStyle.EXPLODE) {
