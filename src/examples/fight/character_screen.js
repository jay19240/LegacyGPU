...
class CharacterScreen extends Screen {
  ...
  async onEnter(args = { map }) {
    ...
    this.uiBackground.src = './examples/fight/ui/bg-map-screen.jpg';
    ...
    for (let i = 0; i < this.numPlayers; i++) {
      ...
      this.uiAvatars[i].src = 'examples/fight/chars/' + 'default-avatar.png';
    ...
    for (let i = 0; i < CHARS.length; i++) {
      this.uiMenu.addWidget(new UIWidget({
        ...
        <div class="UICharacter-container">
          <img class="UICharacter-container-char-icon js-icon" alt="icon" src="${'examples/fight/chars/' + CHARS[i]}/avatar.jpg"/>
        </div>`
      }));
    }
  ...
  async handleMenuItemFocused(data) {
    this.uiAvatars[this.selectionPlayerIndex].src = 'examples/fight/chars/' + CHARS[data.index] + '/avatar.jpg';
  }
  ...
  async handleMenuItemSelected(data) {
    ...
    if (this.selectionPlayerIndex == this.numPlayers) {
      ...
      this.uiAvatars[this.selectionPlayerIndex].src = 'examples/fight/chars/' + CHARS[index] + '/avatar.jpg';
    }
  }
}
