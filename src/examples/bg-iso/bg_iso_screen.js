...
import { Gfx2TileIsoMapLayer } from '@lib/gfx2_tile_iso/gfx2_tile_iso_map_layer';
...
class BgIsoScreen extends Screen {
  constructor() {
    ...
    this.layer1 = new Gfx2TileIsoMapLayer();
    this.gridIds = [];
    this.gridCollisions = [];
    this.gridLayers = [];
    this.gridSubSprites = [];
  ...
  async onEnter() {
    await this.tilemap.loadFromFile('./examples/bg-iso/tilemap.json');
    this.layer1.loadFromTileMap(this.tilemap, 0);
    this.layer1.setShowDebug(true);

    this.bg0.setTexture(await gfx2TextureManager.loadTexture('./examples/bg-iso/background.png'));
